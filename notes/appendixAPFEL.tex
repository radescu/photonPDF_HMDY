...
the different definitions of the expansion parameters one finds:
...
the photon PDF without including the leptons.
%
...
$q = u,d,s,c,b,t$. In addition, we have that:
\begin{equation}
\begin{array}{ll}
...
Eq.~(\ref{eq:EvolBasis}).
%
To this end, we split each splitting function $P$ into a pure QCD term
$\widetilde{P}$, which only depends on $\alpha_s$, and a mixed QCD+QED
correction term $\overline{P}$, which instead contains contributions
proportional to at least one power of the QED coupling $\alpha$.
%
...
%
There are two particularly relevant new features that distinguish
these corrections from the $\mathcal{O}(\alpha)$ and the
...
corrections:
\begin{itemize}
\item the LO QED corrections $\mathcal{O}(\alpha)$ only,
...
\end{itemize}
In Fig.~\ref{fig:GammaGammaLumi}, the results are shown normalised to
the predictions obtained with LO QED corrections only.
%
...
\caption{The photon-photon PDF luminosity $\mathcal{L}_{\gamma\gamma}$ at $\sqrt{s} = 13$ TeV as a
  ...
  with the corresponding results taking into account either
...
The main complication of the inclusion of these corrections to the DIS
structure functions arises from their flavour structure. In fact, due
...
%
Referring $e.g.$ to expressions reported in Ref.~\cite{Ellis:1991qj},
one has that:
...
computation at NLO. The plots are produced by evolving the NNPDF3.0QED
NLO set from $Q_0=1$ GeV to $Q=100$ GeV including the NLO QCD+QED
corrections discussed in the previous section and using the resulting
evolved PDFs to compute the NC (left panel) and the CC (right panel)
DIS structure functions in the FONLL-B scheme (NLO) including the
$\mathcal{O}(\alpha)$ corrections to the coefficient functions
discussed above.
%
...
$F_2, F_3$ and $F_L$, normalised to the pure QCD result.
%
